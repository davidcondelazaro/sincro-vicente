alter table public.catalog_import_runs
  ...
  add constraint catalog_import_runs_entity_type_check check (entity_type in ('manufacturers', 'categories', 'features', 'products', 'icecat'));

alter table public.catalog_import_events
  drop constraint catalog_import_events_entity_type_check,
  add constraint catalog_import_events_entity_type_check check (entity_type in ('manufacturers', 'categories', 'features', 'products', 'icecat'));
...
begin
  ...
  if p_entity_type = 'manufacturers' then
    ...
    select count(*) into v_total from public.source_features x where x.id not in ('10603','10604','10606','10607','10608','10609','10593','10454','10421','10419','10420','10530','10543','10428','PORTESGRATIS','DESCATALOGADO','OFERTA','SUPEROFERTA','PRECIOOCULTO','PRIORIDAD') and (nullif(v_filters ->> 'featureId', '') is null or x.id = v_filters ->> 'featureId');
    ...
    where (coalesce((v_filters ->> 'onlyActive')::boolean, false) is false or x.active is true)
      and ((jsonb_typeof(v_filters -> 'productIds') = 'array' and x.id in (select jsonb_array_elements_text(v_filters -> 'productIds'))) or (jsonb_typeof(v_filters -> 'productIds') <> 'array' and (nullif(v_filters ->> 'productId', '') is null or x.id = v_filters ->> 'productId')))
  ...
  if v_total = 0 then raise exception 'No hay productos con EAN para el criterio indicado'; end if;
