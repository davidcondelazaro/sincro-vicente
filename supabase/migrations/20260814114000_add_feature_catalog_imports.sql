alter table public.catalog_import_runs
  ...
  add constraint catalog_import_runs_entity_type_check check (entity_type in ('manufacturers', 'categories', 'features'));

alter table public.catalog_import_events
  drop constraint catalog_import_events_entity_type_check,
  add constraint catalog_import_events_entity_type_check check (entity_type in ('manufacturers', 'categories', 'features'));
...
begin
  ...
  if p_entity_type not in ('manufacturers', 'categories', 'features') then raise exception 'Entidad no disponible'; end if;
  if jsonb_typeof(v_filters) <> 'object' then raise exception 'Filtros no válidos'; end if;
  if p_entity_type = 'manufacturers' then select count(*) into v_total from public.source_manufacturers x where (coalesce((v_filters ->> 'onlyActive')::boolean, true) is false or x.active is true) and (nullif(v_filters ->> 'manufacturerId', '') is null or x.id = v_filters ->> 'manufacturerId') and (nullif(v_filters ->> 'name', '') is null or x.name ilike '%' || (v_filters ->> 'name') || '%');
  elsif p_entity_type = 'categories' then select count(*) into v_total from public.source_categories x where (nullif(v_filters ->> 'categoryId', '') is null or x.id = v_filters ->> 'categoryId') and (nullif(v_filters ->> 'name', '') is null or x.name ilike '%' || (v_filters ->> 'name') || '%');
  else select count(*) into v_total from public.source_features x where x.id not in ('10603','10604','10606','10607','10608','10609','10593','10454','10421','10419','10420','10530','10543','10428','PORTESGRATIS','DESCATALOGADO','OFERTA','SUPEROFERTA','PRECIOOCULTO','PRIORIDAD') and (nullif(v_filters ->> 'featureId', '') is null or x.id = v_filters ->> 'featureId') and (nullif(v_filters ->> 'name', '') is null or x.name ilike '%' || (v_filters ->> 'name') || '%');
  end if;
