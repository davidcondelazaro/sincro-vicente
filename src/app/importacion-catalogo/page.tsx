...
type Run = { id: string; entity_type: Entity; filters: { onlyActive?: boolean; productSyncMode?: "changes" | "all"; manufacturerId?: string; categoryId?: string; featureId?: string; productId?: string; productIds?: string[]; modifiedSince?: string; forceImages?: boolean; name?: string }; status: "queued" | "running" | "paused" | "stopped" | "completed" | "failed"; total_count: number; processed_count: number; created_count: number; updated_count: number; unchanged_count: number; unpublished_count: number; error_count: number; created_at: string; started_at: string | null; finished_at: string | null };
type ImportEvent = { id: number; run_id: string; level: "info" | "success" | "warning" | "error"; outcome: "created" | "updated" | "unchanged" | "unpublished" | "error" | "status"; source_entity_id: string | null; source_entity_name: string | null; shopify_resource_id: string | null; message: string; details: Record<string, unknown> | null; created_at: string };
...
export default function CatalogImportPage() {
  ...
  const [productSyncMode, setProductSyncMode] = useState<"changes" | "all">("changes");
  ...
  async function start(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/catalog/runs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ entityType, manufacturerId, categoryId, featureId, productId, modifiedSince, forceImages, productSyncMode, onlyActive }) });
  ...
  return <div className="min-h-screen bg-zinc-50 lg:grid lg:grid-cols-[15rem_minmax(0,1fr)]">
    ...
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-7 px-6 pb-24 pt-28 sm:px-10 lg:mx-0 lg:py-16">
      ...
      <style>{`
        ...
        select option[value="priorities"] { display: none; }
      `}</style>
