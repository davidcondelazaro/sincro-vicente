...
function filters(input: Record<string, unknown>) {
  ...
  if (entityType !== "manufacturers" && entityType !== "categories" && entityType !== "features" && entityType !== "products" && entityType !== "icecat") throw new Error("La entidad seleccionada no está disponible todavía.");
  ...
  return { entityType, filters: { onlyActive: input.onlyActive === true, forceImages: input.forceImages === true, ...(productIds.length ? { productIds } : {}), ...(modifiedSince ? { modifiedSince: `${modifiedSince}T00:00:00.000Z` } : {}) } };
}
...
export async function GET(request: Request) {
  ...
  const pageSize = 15;
  if (entityType && entityType !== "manufacturers" && entityType !== "categories" && entityType !== "features" && entityType !== "products" && entityType !== "icecat") return Response.json({ error: "Entidad no válida." }, { status: 400 });
...
export async function POST(request: Request) {
  try {
    ...
    try {
      const worker = input.entityType === "icecat" ? "sync-icecat-imports" : "sync-catalog-imports";
