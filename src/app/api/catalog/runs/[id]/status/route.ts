...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.rpc("set_catalog_import_status", { p_run_id: id, p_status: status });
  if (error || !data) return Response.json({ error: error?.message ?? "No se pudo actualizar la ejecución." }, { status: 400 });
  const { data: run } = await supabase.from("catalog_import_runs").select("entity_type").eq("id", id).maybeSingle();
  const worker = run?.entity_type === "icecat" ? "sync-icecat-imports" : "sync-catalog-imports";
  await supabase.functions.invoke(worker, { body: {} });
